...
export function createSupabaseDatabase(url, serviceRoleKey, fetcher = fetch) {
  ...
  const roomFromDb = row => row ? ({ roomId: row.room_id, roomCode: row.room_code, roomName: row.room_name, adminPasswordHash: row.admin_password_hash, createdAt: row.created_at, updatedAt: row.updated_at }) : null
