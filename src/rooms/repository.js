...
export function createSupabaseRoomRepository({ url, publicKey = '', fetcher = fetch }) {
  ...
  return {
    ...
    enterRoom: (roomId, roomCode) => call('enter', { roomId, roomCode }),
    listRooms: roomCodes => call('list', { roomCodes }),
    listAllRooms: (offset = 0) => call('list-all', { offset }),
    renameRoom: (roomCode, token, password, roomName, icon) => call('rename', { roomCode, token, password, roomName, icon }),
