...
export function PasswordDialog({ mode, title, onSubmit, onCancel, cloud = false }) {
  ...
  return <dialog ref={ref} className="notice-dialog password-dialog" aria-labelledby={`${id}-title`} aria-describedby={`${id}-description`} onCancel={event => { event.preventDefault(); if (!busy) onCancel() }}>
    ...
    <form onSubmit={async event => {
      ...
      <label>비밀번호<input className="text-input" type="password" value={password} minLength={creating ? 8 : undefined} maxLength={128} autoComplete={creating ? 'new-password' : 'current-password'} autoFocus required disabled={busy} onChange={event => setPassword(event.target.value)} /></label>
      {creating && <label>비밀번호 확인<input className="text-input" type="password" value={confirmation} minLength={8} maxLength={128} autoComplete="new-password" required disabled={busy} onChange={event => setConfirmation(event.target.value)} /></label>}
      {creating && <p className="allocation-help">8~128자 · 비밀번호를 잊으면 이 기록을 수정할 수 없습니다.</p>}
