...
function EditorPage({ data, onChange, recordsStore, onSaved, editing = false, onCancel, onImport }) {
  ...
  const prepareSave = () => {
    try {
      ...
      setError('')
      if (editing) { onSaved(); setNotice('변경 내용을 저장했습니다.') }
      else setSaving(true)
  ...
  return <main id="main" className="page-container service-page">
    ...
    <div className="editor-toolbar"><label>정산 날짜<input className="text-input" type="date" value={data.date} onChange={event => onChange({ ...data, date: event.target.value })} /></label><div>{editing && <button className="button button-secondary" onClick={onCancel}>수정 취소</button>}<button className="button button-primary" onClick={prepareSave}>{editing ? '수정 내용 저장' : '정산 저장'}</button></div></div>
    <div className="rule-notice"><span className="fee-tag">수수료 5%</span><p>택배·교환 모두 동일 · 사고는 해당 트라이에만 적용</p></div>
    {(error || recordsStore.error) && <p className="field-error page-message" role="alert">{error || recordsStore.error}</p>}
    {notice && <p className="success-message" role="status">{notice}</p>}
    <SettlementEditor data={data} onChange={onChange} onImport={loadRecent} />
    <div className="editor-bottom-actions"><p className="draft-note">‘{editing ? '수정 내용 저장' : '정산 저장'}’을 누르면 브라우저를 다시 열어도 기록이 유지됩니다. 저장 전 입력·변경은 새로고침하면 사라집니다.</p><button className="button button-primary" onClick={prepareSave}>{editing ? '수정 내용 저장' : '정산 저장'}</button></div>
    {saving && <PasswordDialog mode="create" onCancel={() => setSaving(false)} onSubmit={async password => { const saved = await recordsStore.repository.create(data, password); recordsStore.refresh(); setSaving(false); onSaved(saved) }} />}
  </main>
}
