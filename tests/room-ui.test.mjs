...
test('UI: room create/share/read/admin edit, plus chaos party selector/filter and Korean inputs', async () => {
  ...
  try {
    ...
    await mount()
    assert.equal(document.querySelector('.room-icon-preview').textContent, '🍁')
    await click(button('아이콘 변경'))
    assert.equal(document.querySelectorAll('.room-icon-options button').length, 17)
    await click(label('아이콘 🍄'))
    ...
    await wait(() => document.querySelector('.room-directory-card'))
    assert.match(document.querySelector('.room-directory-card').textContent, /🍄.*UI 공대방/s)
    assert.equal(document.querySelector('.room-directory-card a').getAttribute('href'), `#/room/${code}`)
    ...
    assert.match(document.querySelector('.sale-status').textContent, /판매 전/)
    await input(label('1트 아이템 1 판매금액'), '850000000')
    assert.equal(document.querySelector('.sale-status').textContent, '판매 완료')
    ...
    await click(button('+ 아이템 추가'))
    const names = document.querySelectorAll('[placeholder="아이템 검색 또는 직접 입력"]')
    await input(names[names.length - 1], '판매 예정 아이템')
    ...
    assert.equal(document.querySelectorAll('.sale-status-sold').length, 1)
    assert.equal(document.querySelectorAll('.sale-status-pending').length, 1)
    ...
    await navigate(`/room/${code}`)
    await wait(() => button('관리자 비밀번호 확인'))
    await click(button('관리자 비밀번호 확인'))
    await input(document.querySelector('input[type="password"]'), 'test1234')
    await click(button('확인', document.querySelector('dialog')))
    await wait(() => document.querySelector('.room-danger'))
    const recordsBeforeRename = JSON.parse(dom.localStorage.getItem(ROOMS_KEY)).settlements
    await click(button('정산방 이름 수정'))
    await input(document.querySelector('dialog input:not([type="password"])'), '이름 변경 공대')
    await click(button('아이콘 변경', document.querySelector('dialog')))
    await click(label('아이콘 🍀'))
    await input(document.querySelector('dialog input[type="password"]'), 'wrong')
    await click(button('변경하기', document.querySelector('dialog')))
    await wait(() => document.querySelector('dialog .field-error'))
    assert.equal(JSON.parse(dom.localStorage.getItem(ROOMS_KEY)).rooms[0].roomName, 'UI 공대방')
    await input(document.querySelector('dialog input[type="password"]'), 'test1234')
    await click(button('변경하기', document.querySelector('dialog')))
    await wait(() => document.querySelector('h1')?.textContent.includes('이름 변경 공대'))
    assert.equal(document.querySelector('.room-current-icon').textContent, '🍀')
    await wait(() => document.querySelector('.room-directory-card h3')?.textContent === '이름 변경 공대')
    assert.match(document.querySelector('.save-toast').textContent, /정산방 정보가 변경되었습니다/)
    assert.equal(document.querySelector('.room-directory-card .room-icon-preview').textContent, '🍀')
    assert.deepEqual(JSON.parse(dom.localStorage.getItem(ROOMS_KEY)).settlements, recordsBeforeRename)
    assert.match(document.querySelector('.all-rooms').textContent, /공개 정산방 목록은 클라우드 모드/)
    await click(document.querySelector('.room-danger summary'))
    await click(button('정산방 삭제'))
    assert.match(document.querySelector('dialog').textContent, /복구할 수 없습니다/)
    await input(document.querySelector('dialog input[type="password"]'), 'wrong')
    await click(button('삭제', document.querySelector('dialog')))
    await wait(() => document.querySelector('dialog .field-error'))
    assert.equal(JSON.parse(dom.localStorage.getItem(ROOMS_KEY)).rooms.length, 1)
    await input(document.querySelector('dialog input[type="password"]'), 'test1234')
    await click(button('삭제', document.querySelector('dialog')))
    await wait(() => document.querySelector('h1')?.textContent === '우리 공대의 정산방')
    assert.equal(JSON.parse(dom.localStorage.getItem(ROOMS_KEY)).settlements.length, 0)
    await navigate(`/room/${code}`)
    await wait(() => document.body.textContent.includes('정산방을 찾을 수 없습니다'))
  } finally {
