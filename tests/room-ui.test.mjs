...
test('UI: room create/share/read/admin edit, plus chaos party selector/filter and Korean inputs', async () => {
  ...
  try {
    ...
    await input(createInputs[0], 'UI 공대방'); await input(createInputs[1], '123'); await input(createInputs[2], '123')
    await click(button('정산방 생성'))
    assert.match(document.querySelector('.save-error').textContent, /4~8자리/)
    assert.equal(dom.localStorage.getItem(ROOMS_KEY), null)
    await input(createInputs[0], 'UI 공대방'); await input(createInputs[1], 'test1234'); await input(createInputs[2], 'test1234')
    await click(button('정산방 생성'))
    ...
    assert.match(document.querySelector('.operating-fund-total').textContent, /공대 운영금.*0/s)
    const fundSelect = label('1트 세영 사망 차감')
    assert.ok([...fundSelect.options].some(option => option.textContent === '50% 차감 (흰경/공대 운영금)'))
    await input(fundSelect, 'operating-fund')
    assert.match(document.querySelector('.operating-fund-total').textContent, /403,750,000/)
    ...
    assert.equal(label('1트 세영 사망 차감').value, 'operating-fund')
    assert.match(document.querySelector('.operating-fund-total').textContent, /403,750,000/)
    ...
    await input(document.querySelector('input[type="password"]'), 'test1234'); await click(button('확인', document.querySelector('dialog')))
